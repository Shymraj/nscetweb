...
db.connect(err => {
  ...
  const depts = [
    { srcName: 'ECE', dbName: 'Electronics and Communication Engineering' },
    { srcName: 'EEE', dbName: 'Electrical and Electronics Engineering' },
    { srcName: 'MECH', dbName: 'Mechanical Engineering' },
    { srcName: 'CIVIL', dbName: 'Civil Engineering' }
  ];
  ...
  depts.forEach(dept => {
    ...
    const files = fs.readdirSync(srcDir).filter(f => f.match(/\.(jpg|jpeg|png|gif|webp)$/i));
