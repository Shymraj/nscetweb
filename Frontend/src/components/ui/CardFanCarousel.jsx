...
import { FaImages, FaCalendarAlt, FaArrowRight } from "react-icons/fa";
import "./CardFanCarousel.css"; // We will create this for layout classes
...
export default function CardFanCarousel({ events, getCategory }) {
  ...
  // Reset animation state when events change (like changing filters)
  useEffect(() => {
    setCenterIndex(needsPagination ? HALF : totalCards >> 1);
    hasEntered.current = false;
    isAnimating.current = false;
    prevVisible.current = new Set();
  }, [events, needsPagination, totalCards]);

  const getVisibleMap = useCallback((center) => {
    const map = new Map();
    if (!needsPagination) {
      events.forEach((_, i) => map.set(i, i));
      return map;
    }
    ...
    return map;
  }, [totalCards, needsPagination, events]);
  ...
  const chevron = (direction) => (
    <svg className="fan-chevron-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      <polyline points={direction === "left" ? "15 18 9 12 15 6" : "9 18 15 12 9 6"} />
    </svg>
  );

  return (
    <section className="fan-carousel-section">
      <div className="fan-carousel-container">
        <div ref={containerRef} className="fan-layout">
          {events.map((event, index) => {
            ...
            // Recreate the exact card HTML from the original
            const cardContent = (
              <div className="structured-event-card h-full">
            ...
            return (
              <div key={event.id || event.slug} className="fan-card fan-card-wrapper">
                {cardContent}
              </div>
            );
          })}
      ...
      {needsPagination && (
        <div className="fan-pagination">
          ...
          <div className="fan-dots">
            {events.map((_, i) => (
              <span key={i} className={`fan-dot ${i === centerIndex ? "active" : ""}`} />
            ))}
          </div>
