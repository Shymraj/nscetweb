...
const StaffManager = () => {
  ...
  const getDeptSlug = (dept) => {
    ...
    if (d.includes('me-cse') || d.includes('m.e. - computer') || d.includes('m.e. computer')) return 'me-cse';
    if (d.includes('me-structural') || d.includes('m.e. - structural') || d.includes('m.e. structural')) return 'me-structural';
    if (d.includes('me-manufacturing') || d.includes('m.e. - manufacturing') || d.includes('m.e. manufacturing')) return 'me-manufacturing';
    if (d.includes('me-embedded') || d.includes('m.e. - embedded') || d.includes('m.e. embedded')) return 'me-embedded';
  ...
  const handleOpenAcademicModal = (st) => {
    ...
    const slug = getDeptSlug(st.department);
    const rawFallback = getFacultyData(slug, st.name);
    const normalize = (str) => (str || "").replace(/dr\.|mr\.|mrs\.|ms\./gi, '').replace(/[^a-z0-9]/gi, '').toLowerCase();
    const isSamePerson = rawFallback && normalize(rawFallback.name) === normalize(st.name);
    const fallback = isSamePerson ? rawFallback : {};
